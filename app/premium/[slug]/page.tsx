...
import { PremiumLicenseTools } from '@/components/PremiumLicenseTools';
import { premiumReports } from '@/lib/content';
import { siteStrings } from '@/src/strings';

interface Props {
  params: { slug: string };
  searchParams?: { via?: string; license?: string };
}
...
export default function PremiumPage({ params, searchParams }: Props) {
  const report = premiumReports.find((item) => item.slug === params.slug);
  if (!report) {
    notFound();
  }

  const isLicensedDisplay = searchParams?.via === 'fairfetch' && searchParams?.license === 'DISPLAY';
  ...
  return (
    <ArticleLayout title={report.title} date={report.date}>
      <div className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-800">
        <p className="font-semibold">🔒 Premium Research</p>
        <p className="mt-2">Human preview: This page shows a short preview with a paywall callout by default.</p>
        <p className="mt-1">Licensed AI view: Add FairFetch query params to show the full report content.</p>
      </div>

      <p className="rounded-lg border border-slate-200 bg-white p-4 text-slate-700">{report.preview}</p>

      <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
        <p>This report is premium for humans. AI access is available via FairFetch licensing.</p>
      </div>

      <PremiumLicenseTools />

      {!isLicensedDisplay ? null : (
        <>
          <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
            <p className="font-semibold">Licensed via FairFetch</p>
            <p className="mt-1">This page is intended to be fetched by licensed AI clients. FairFetch issues a receipt.</p>
          </div>
